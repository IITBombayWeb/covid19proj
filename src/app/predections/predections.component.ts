...
export class PredectionsComponent implements OnInit {
	...
	resetView() {
		this.DropdownState.nativeElement.getElementsByTagName('option')[0].selected = true
		this.def_list = 0
		this.sa_list = 0
		...
		mapdata.then(function (topology) {
			...
			svgEle[1].selectAll('path')
				...
				.on("click", function (d) {
					...
					const n1 = d.properties.st_nm;
					const n2 = d.properties.district;
					let data = clickDisFun(date,n1,n2,
											FunCrtical,SFunCrtical,
											CFunCrtical,btn,btn2,model,
											rmLegendFun,mkLegendFun,
											gsvg,maxd,maxInterpfactor,
											pService,tData,MapFill)
				...
				//dpdist.dname = DdistFun(dpState.sname[0])
			MapFill(model, FunCrtical, maxd,
				date.date,
				btn2.querySelector('.active')
					.getElementsByTagName('input')[0].value,
				maxInterpfactor);
				dpState.sname = DstateFun()
		});
...
changeDropdownDistrict(d){
	const btn1 = this.buttonToggle.nativeElement
	const btn2 = this.buttonToggle2.nativeElement
	this.resetToggel(btn1)
	this.Sdate.date =btn1
	.getElementsByTagName('input')[0].value
	const n1 = d.target[d.target.selectedIndex].dataset.state
	const n2 = d.target.value;
	let maxinter = this.getMaxInterp(this.Sdate.date,this.paramsType)
	let maxd = this.getMaxd(this.model,this.Sdate.date,this.paramsType)
	let data = this.clickDistrict(this.Sdate,n1,n2,
		this.getDistricCrtical,this.getSateCrtical,
		this.getCountryCrtical,btn1,btn2,this.model,this.removeColorLegend
		,this.createLegend,this.Gsvg,maxd,maxinter,this.ps, this.DataTBL,this.setMapColor)
		this.def_list = data[0]
		this.cn_list = data[1]
		this.sa_list = data[2]
		this.dataSource = data[3]
		this.Thead.sname = n1; this.Thead.dname = n2
}
changeDropdownstate(d){
	const newVal = d.target.value;
   this.dropDownListdist = {dname:this.getDistdata(newVal)}
   
   //console.log(this.dropDownListdist.dname)
}
	removeColorLegend() {
		...
		if (this.Thead.dname !== '') {
			this.def_list =
				this.getDistricCrtical(this.model,
					...
					date, this.paramsType)
			this.sa_list =
				this.getSateCrtical(this.model, this.Thead.sname, date, this.paramsType)
	...
	getDistdata(name) {
		...
		state = data.map(dta => dta.sname);
		return data.filter(element => element.sname == name).sort((a, b) => {
			return (b.dname==="Select District")?1:a.dname.localeCompare(b.dname)})
	}
	clickDistrict(date,n1,n2,FunCrtical,SFunCrtical,CFunCrtical,btn,btn2,model,rmLegendFun,mkLegendFun,gsvg,maxd,maxInterpfactor,pService,tData,MapFill){
	//	console.log(maxd,maxInterpfactor,date)
		...
		const dSource = pService.getTableData(numCritical,
			Ccritical,Scritical, tData);
		rmLegendFun()
		...
		MapFill(model, FunCrtical, maxd,
			...
			btn2.querySelector('.active')
				.getElementsByTagName('input')[0].value,
			maxInterpfactor);
		return [numCritical,Ccritical,Scritical,dSource]
	}
	resetToggel(btn) {
