...
export class PredectionsComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.DataMp= [
      {id:"t_kits", name: '', type: 'Test_Kits',map: ''} //,
      // {id:"vents", name: 'Ventilators',  type: 'Ventilators',''},
      // {id:"masks", name: 'Masks',  type: 'Masks'},
    ];
    ...
    this.ps.requestDataFromMultipleSources().subscribe(responseList => {
      this.responseData['t_kits'] = responseList[0];
     // this.responseData['vents'] = responseList[1];
     // this.responseData['masks'] = responseList[2];
      //this.maxcalculate(this.objCOn(this.responseData['t_kits']))
      this.getMAx()
  ...
  getMAx(){
    for(let i =0;i< this.objCOn(this.responseData).length;i++){
      this.max_number[this.DataMp[i].id] = this.maxcalculate(this.objCOn(this.responseData[this.DataMp[i].id]));
  ...
  function zoomed() {
    ...
    let aread = this.responseData[this.DataMp[i].id];
        ...
        g.selectAll('path')
          ...
          .attr('fill', function (d) {
            ...
            const color =
              n === 0
                ? '#ffffff'
                : d3.interpolateReds(
                    (maxInterpolation * n) / ( maxConfirmed|| 0.001)
                  );
            return color;
          })
          ...
          .on('mouseover', (d) => {
           ...
           dSource = pService.getTableData(cnf, m_list);
            tooltip
            .html(d.properties.NAME_1 + "<br>" + "District: " + d.properties.NAME_2 + "<br>" + "Qty: " + cnf)
